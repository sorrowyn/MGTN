...
parser = argparse.ArgumentParser(description='MGTN Training')
...
def main_coco():
    ...
    if args.neptune:
        with open(args.neptune_path, 'r') as f:
            ...
            neptune.create_experiment(params=vars(args), upload_source_files=[
                                      'demo_coco_gcn.py', 'coco.py', 'engine.py', 'models.py', 'util.py'])

    train_dataset = COCO2014(args.data, phase='train', inp_name=args.embedding)
    val_dataset = COCO2014(args.data, phase='val', inp_name=args.embedding)
    num_classes = 80
